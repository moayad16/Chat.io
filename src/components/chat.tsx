...
export default function Chat({ fileKey }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full lg:px-12 py-5">
      <div
      ...
      >
        {messages.map((message, index) => {
          if (message.role === "assistant") {
            return (
              <div className="bg-blue-600 h-fit lg:max-w-60% py-2 px-4 mr-auto rounded-xl mb-9">
                {message.content}
              </div>
            );
          } else {
            return (
              <div className="bg-blue-400 lg:max-w-60% w-fit px-4 py-2 rounded-xl mb-9 ml-auto">
                {message.content}
              </div>
            );
          }
